-- ==============================================================================
-- 🚀 SCRIPT COMPLETO DE CONFIGURAÇÃO DO SISTEMA (SUPABASE)
-- Este script cria todas as tabelas, colunas e configurações necessárias.
-- É seguro rodar este script múltiplas vezes (ele não apaga dados existentes).
-- ==============================================================================
...
-- Garantir que a coluna plainPassword existe (para atualizações)
DO $$
BEGIN
...
-- Hash de 'admin' gerado via bcrypt (exemplo) ou texto simples se o sistema suportar
-- Aqui usamos um hash placeholder, mas o backend deve lidar com a criação se não existir.
-- Este bloco insere apenas se não existir.

INSERT INTO public.users (id, email, name, role, approved, password, "plainPassword")
VALUES (
    'admin_default', 
    'admin@sistema.mz', 
    'Super Administrador', 
    'super_admin', 
    true, 
    '$2a$10$X7V.j.X7V.j.X7V.j.X7V.j.X7V.j.X7V.j.X7V.j.X7V.j.X7V.j.', -- Placeholder hash
    'admin'
)
